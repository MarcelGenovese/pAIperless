...
// Audit log function
async function auditLog(action, details, user = 'cli') {
  try {
...
async function resetPaperlessToken(token) {
  if (!token || token.trim() === '') {
...
async function resetPaperlessUrl(url) {
  if (!url || url.trim() === '') {
...
async function getConfigValue(key, showSecret = false) {
  try {
